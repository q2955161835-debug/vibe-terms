...
当前 12 词原型只证明了路线和本地学习可运行，不代表内容与体验完成。下一阶段的首个可交付里程碑是 60 个高频核心词、3 条可执行项目路径、每词小练和分层示例；长期目标仍是 500 个规范词条、8 个语言版本。
...
| 分类深度 | 7 个一级领域，缺少稳定二级目录 | 8 个一级领域 + 可扩展二级主题 + 数量与交叉筛选 |
...
| 内容规模 | 12 个代表词 | 首个里程碑 60；随后 180；长期 500 |
...
| --- | ---: | ---: | --- | --- |
| 核心体验 | 60 | 3 | 每词有示例模块，至少 20 个互动/分步示例 | 每词至少 1 题 |
| 领域覆盖 | 180 | 8 | 高频流程与界面词优先互动 | 每词至少 1 题，核心词 2 题 |
| 完整目标 | 500 | 12+ | 仅在提高理解时增加互动 | 每词至少 1 题，路径节点有综合题 |
...
| 风险 | 缓解 |
| --- | --- |
| 追求 500 词导致低质量批量内容 | 先做 60，再按 8–12 词批次；每批必须过内容门禁 |
...
在用户确认网页整理文件已经传达前，本轮只保存本设计与配套实施计划，不修改 `content/`、`scripts/`、`web/`、`app/`、`worker/` 或部署工作流。
